...
```
7) Now lets add an animation to the box to rotate it using the animation defined in rotate.  We can specify the duration of the animation and the state for the animation.  Notice that we are defining a different set of keyframes based on rotation.
```html
.box {
  background-color: rebeccapurple;
  border-radius: 10px;
...
12) Now put everything inside of the <style> tag into a file called style.css and add a link tag in the <head>
```
